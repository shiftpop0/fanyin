...
// @version      0.4.4
// @description  捕获 spyware 页面语音切片，合并后调用 Tailect ASR v1，保存 CSV，并在列表/VX 页面展示和修正。
...
(function () {
  ...
  const STORAGE_KEY = 'tailect_asr_translator_v2_settings';
  const SETTINGS_SCHEMA_VERSION = 3;
  const PANEL_ID = 'tailect-asr-translator-panel';
  const MODAL_ID = 'tailect-asr-translator-modal';
  const TOAST_ID = 'tailect-asr-translator-toasts';
  ...
  const LOCAL_HELPER_BAT = 'spyware-translator\\local_helper\\启动本机CSV助手.bat';
  const CSV_COLUMNS = [
    ['record_key', '记录键'],
    ['scene', '场景'],
    ['title', '标题'],
...
`;
  ...
  async function hydrateTaskFromLocalCsv(task, options) {
    ...
    try {
      ...
      if (status && status.exists && isChineseCsv(status.csv_text || '')) {
  ...
  function canSaveTaskCsv(task) {
    return Boolean(task && (
      isChineseCsv(task.localCsvText || '') ||
  ...
  async function resolveTaskCsvForSaveAs(task) {
    ...
    if (isChineseCsv(task.localCsvText || '')) return task.localCsvText;
    ...
    if (!result || !result.exists || !isChineseCsv(csvText)) {
  ...
  async function checkModelService(force, notify) {
    ...
    try {
      ...
      state.modelService.ok = Boolean(result && (result.status === 'ok' || result.code === 200));
      if (!state.modelService.ok) throw new Error(result && result.message ? result.message : '健康检查未返回正常状态');
      state.modelService.message = '已连接';
      clearPanelNotice('model');
      ...
      <div class="tm-controls">
        <section class="tm-config-section">
          ...
          <div class="tm-form-grid">
            ...
            <select name="model">
              <option value="Taizhou" ${settings.model === 'Taizhou' ? 'selected' : ''}>Taizhou</option>
              <option value="Tiantai" ${settings.model === 'Tiantai' ? 'selected' : ''}>Tiantai</option>
            </select>
